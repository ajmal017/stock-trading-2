...
class TrainingStockEnv(Env):
    ...
    def __init__(self, stock_list, feature_generator, action_space, gym_agent, continuous=False):
        ...
        self.features_over_time = [[] for _ in range(len(feature_generator))]
    ...
    def _next_observation(self):
        ...
        for i in range(len(features)):
            self.features_over_time[i].append(features[i])
    ...
    def save_feature_distribution(self, agent_name):
        ...
        plots = []
        index = 0
        N = len(self.features_over_time)
        cols = 2
        for i in range(N // cols):
            for j in range(cols):
                if index < N:
                    ax = plt.subplot2grid((N // cols, cols), (i,j))
                    ax.hist(self.features_over_time[index])
                    index += 1
